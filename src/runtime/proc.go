...
func schedinit() {
	...
	// 设置 p 个数
	// 这里1.8版有一个_MaxGomaxporcs变量来控制总的 p 个数不超过 256，目前已经去掉了这个限制
	sched.lastpoll = uint64(nanotime())
	procs := ncpu // 默认值为 cpu 总核心数
	if n, ok := atoi32(gogetenv("GOMAXPROCS")); ok && n > 0 {
		procs = n // 使用GOMAXPROCS变量覆盖个数
	}
...
//
// 当 p 到达 GC 安全点的时候，每个P都会调用 fn(p)
// 当 P 正在执行代码时会将这个 P 带入一个 GC 的安全点，并且在 P 上执行 fn
// 当 P 没有在执行代码(处于idle状态或一个系统调用状态), 会直接执行 fn(p)
// 防止 P 退出他的状态
//go:systemstack
...
func execute(gp *g, inheritTime bool) {
	...
	// 执行, 这个是汇编实现的，可以参考runtime/asm_amd64.s 中 gogo的实现
	gogo(&gp.sched)
}
...
top:
	...
	// 如果 g 需要 lock 到 m 上，则会将当前的 p 给这个要 lock 的 g, 然后阻塞等待一个新的 p
	if gp.lockedm != 0 {
...
// 当goroutine 执行完后，执行goexit, goexit执行goexit1
...
func goexit0(gp *g) {
	...
	// wasm 目前还没有线程支持, 将 g 扔进 gfree 链表中等待复用
	if GOARCH == "wasm" { // no threads yet on wasm
		gfput(_g_.m.p.ptr(), gp)
		schedule() // never returns 再次进入调度
	}
	...
	if locked {
		...
		if GOOS != "plan9" { // See golang.org/issue/22227.
			gogo(&_g_.m.g0.sched) // 调用 gogo 恢复到 g0.sched 的执行现场，从而恢复到 mexit 调用。
		} else {
...
func malg(stacksize int32) *g {
	newg := new(g)
	if stacksize >= 0 {
		// 根据系统不同，分配的大小不同, 最终分配的大小是大于 (_StackSystem + stacksize) 的最小的2的n次方
...
func newproc(siz int32, fn *funcval) {
	...
	gp := getg()
	pc := getcallerpc() // 获取调用者的pc, 即将要执行的下一条指令的地址
	systemstack(func() {
...
func newproc1(fn *funcval, argp *uint8, narg int32, callergp *g, callerpc uintptr) {
	...
	// 为啥不知直接 _StackMin - 5*sys.RegSize ?
	if siz >= _StackMin-4*sys.RegSize-sys.RegSize {
		throw("newproc: function arguments too large for new goroutine")
	}

	_p_ := _g_.m.p.ptr()
	newg := gfget(_p_) // 从自由 g 列表中获取一个 g
	if newg == nil {   //没有获取到自由g, 重新建立一个g
		newg = malg(_StackMin)           // 创建一个拥有_StackMin(目前是2KB)大小的栈的g, 注意真正分配的栈的大小还要加上系统本身占用的空间
		casgstatus(newg, _Gidle, _Gdead) // 将新创建的 g 从 _Gidle 更新为 _Gdead 状态
		allgadd(newg)                    // publishes with a g->status of Gdead so GC scanner doesn't look at uninitialized stack. 将 Gdead 状态的 g 添加到 allg，这样 GC 不会扫描未初始化的栈
	}
	if newg.stack.hi == 0 {
	...
	newg.sched.sp = sp
	newg.stktopsp = sp
	// 将 goexit 函数地址放到pc寄存器中
	newg.sched.pc = funcPC(goexit) + sys.PCQuantum // +PCQuantum so that previous instruction is in same function
	newg.sched.g = guintptr(unsafe.Pointer(newg))
	gostartcallfn(&newg.sched, fn) // 这里把fn放入到PC寄存器中，当执行完fn后，PC寄存器恢复为 goexit, 所以执行fn后会执行 goexit
	// 初始化 g 的基本状态
	newg.gopc = callerpc
...
// 从 自由 g 列表中获取一个 g
// 当本地的 gFree为空时，从调度器的 gFree获取一个，放到本地的gFree中，然后返回本地gFree中的g
// Get from gfree list.
...
func procresize(nprocs int32) *p {
	...
	for i := nprocs; i < old; i++ {
		...
		p.gcAssistTime = 0
		p.status = _Pdead // 释放的 p 状态为  _PDead
	...
	if _g_.m.p != 0 && _g_.m.p.ptr().id < nprocs {
		...
		_g_.m.p.ptr().status = _Prunning // 当前 g 所绑定的 p 设置状态为 _Prunning
		...
		p := allp[0]
		p.m = 0
		p.status = _Pidle // 当前 p 未与任何 m 关联
		acquirep(p)       // 直接将 p 绑定到当前的 M
		if trace.enabled {
			traceGoStart()
		}
	}
...
func retake(now int64) uint32 {
	...
	for i := 0; i < len(allp); i++ {
		...
		if s == _Psyscall {
			...
			incidlelocked(-1)
			if atomic.Cas(&_p_.status, s, _Pidle) { // 将 P 设为 idle，从而交给其他 M 使用
				if trace.enabled {
					traceGoSysBlock(_p_)
					traceProcStop(_p_)
				}
...
// 不需要锁定。
// 如果发出抢占请求，则返回true。
// 实际的抢占将在未来的某个时刻发生，并且将由gp->状态表示不再是Grunning
func preemptone(_p_ *p) bool {
	...
	// 进入newstack函数，主动弃权
	// 上面的检查只会在函数调用开始的时候进行
	// 如果执行一个死循环，内部没有函数调用则抢占标记没有起作用
	// 这个地方引起的BUG, 参考:
	// https://gocn.vip/article/441 这个说是 gcwaiting 导致的程序hold, 感觉有问题: preemptone中没有设置这个值的地方
	// http://t.cn/AiKJNVd8 这个说法感觉更有说服力
...
// 当要将一个 g 放入全局队列时，不仅仅只影响它自己， 还会将本地队列中一半的 work 给拿走，然后将他们的执行顺序重新打乱。 再放入全局队列。
